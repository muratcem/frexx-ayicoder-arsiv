...
},
{
  type:'input',
  name:'target',
  message:'[>] Kullanıcı Adı Hedefini Girin (Without @[at]):',
  validate: function(value){
    if(!value) return 'Can\'t Empty';
    return true;
  }
},
{
  type:'input',
  name:'mysyntx',
  message:'[>] İstediğiniz Hedefin Toplamını Girin (ITTYW):',
  validate: function(value){
...
const Excute = async function(User, TargetUsername, mysyntx, sleep){
	...
	/** TRY TO LOGIN **/
	console.log('\n');
	console.log('[?] GİRİŞ YAPMAYI DENEYİN . . .');
	const doLogin = await Login(User);
	console.log(chalk`{bold.green [!] Giriş Başarılı!}`);

	/** TRY TO GET ALL MEDIA **/	
	console.log('[?] MEDYA ALMAYI DENEYİN . . .')		
	...
	for (let i = 0; i < getMedia.length; i++) {
			console.log('[?] Fotoğrafı / Gecikmeyi Beğenmeyi Deneyin \n')
